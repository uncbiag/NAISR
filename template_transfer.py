...
if __name__ == "__main__":
    ...
    savepath_evo = os.path.join(root_path, 'ShapeMatrixSpecificTemplateTransfer')
    ...
    for age in np.linspace(-1.0, 1.0, 7):
        ...
        for weight0 in np.linspace(-1.0, 1.0, 7):
            ...
            attributes = {'weight': np.array([weight])[None, :], 'age': np.array([age])[None, :],
                          'sex': np.array([1])[None, :]}
            attributes = {key: torch.from_numpy(value).to(device).float()[[0], ...] for key, value in
                          attributes.items()}

            savedir = os.path.join(savepath_evo_id, 'age_' + str(age) + '_weight_' + str(weight))
            cond_mkdir(savedir)
            savepath = naisr_meshing.create_mesh_3dairway_reconstruction_with_transferred_template(model,
                                                                         average_latent_code,
                                                                         attributes,
                                                                         {},
                                                                         savedir,
