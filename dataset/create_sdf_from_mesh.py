...
def get_canonical_3Dshape(source_subj,
    ...
    R, T, _ = get_registration_of_landmarks(path_landmark, unitized_segmentation_path) #register_landmarks(np.array(arr_centerline_source), np.array(arr_centerline_template))
    arr_aligned_centerline = apply_registration_to_pts(np.array(arr_centerline_source).T, R, T).T
    arr_aligned_normal = np.matmul(R, np.array(arr_normal_source).T).T
    aligned_surf = apply_registration_to_surface(surf_airway, R, T)
    ...
    savedir_3dshape_gif = os.path.join(savedir_3dshape_subj, source_subj + '.gif')
    savedir_3dshape_stl_npy = os.path.join(savedir_3dshape_subj, source_subj + '_stl.npy')
    aligned_surf.export(savedir_3dshape_stl)
    np.save(savedir_3dshape_stl_npy, np.concatenate((np.array(aligned_surf.vertices), np.array(aligned_surf.vertex_normals)), axis=-1))

    #centroid = aligned_surf.bounding_box.centroid
    #vertices = aligned_surf.vertices - aligned_surf.bounding_box.centroid
    #distances = np.max(np.linalg.norm(vertices, axis=1))
    npz_sdf = calculate_sdf_from_mesh(aligned_surf)

    #npz_sdf[:, [0, 1, 2]] = npz_sdf[:, [0, 1, 2]] * distances + centroid
    #npz_sdf[:, [3]] *= distances
    np.save(savedir_3dshape_sdf, npz_sdf)
    del npz_sdf
    #
    ...
    if VIS:
        ...
        p.add_mesh(aligned_surf.vertices, color='grey', point_size=3,)
        p.add_arrows(aligned_surf.vertices, direction=aligned_surf.vertex_normals, color='yellow', mag=0.3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--subj", type=str, required=False, default=1307,
                        help="Path to the config file of the current experiment (the test data, which you want to compare against the atlas)")
    args = parser.parse_args()

    savedir = '/Users/jyn/jyn/research/projects/NAISR/data2viz/dataset/' #'/playpen-raid/jyn/NAISR/NAISR/examples/pediatric_airway/'#  #
    rootpath_atlas = '/Users/jyn/jyn/research/projects/NAISR/data2viz/dataset/' # '/playpen-raid/jyn/pediatric_atlas/FinalData/AtlasExperimentPipeline/atlas_airway_data_new_v2/' #  # '/playpen-raid/jyn/pediatric_atlas/'
    ...
    list_subjs = os.listdir(rootdir_centerline)
    print("there are " + str(len(list_subjs)) + ' cases in the atlas.')
    list_3dshape_dataset = []
    source_subj = str(args.subj)
    ...
    list_3dshape_dataset = get_canonical_3Dshape(
        ...
        VIS=False,
        template_subj='1035',
        metadatapath="/Users/jyn/jyn/research/projects/NAISR/data2viz/dataset/FilteredControlBlindingLogUniqueScanFiltered_19Sep2022.xls",
        #"/playpen-raid/jyn/pediatric_atlas/FinalData/AtlasExperimentPipeline/atlas_airway_data_new_v2/FilteredControlBlindingLogUniqueScanFiltered_19Sep2022.xls",
        # "/playpen-raid/jyn/pediatric_atlas/FinalData/AtlasExperimentPipeline/atlas_airway_data_new_v2/FilteredControlBlindingLogUniqueScanFiltered_19Sep2022.xls",
        savepath_dataset='./3dshape.csv')
    print(source_subj)
