...
def transport(
    ...
    optimizer_lat = torch.optim.Adam([latent], lr=lr)
    ...
    for e in range(num_iterations):
        ...
        model_output = decoder(xyz, attributes_unknown, latent_inputs[None, :, :], testing=True,training=False)
...
if __name__ == "__main__":
    ...
    for transport_name in ['all', 'age', 'weight']:
        ...
        for current_patient in list_patient_scans:
            ...
            cond_mkdir(transport_general_covariate_meshes_dir_subj)
            ...
            if not save_latvec_only:
                latent_filename = os.path.join(transport_general_codes_dir, str(test_idx) + ".pth")
                inferred_covariate_filename = os.path.join(transport_general_codes_dir,
            ...
            for ith_scan_to_transp in other_scans:
                ...
                if not save_latvec_only:
                    latent_filename = os.path.join(transport_general_codes_dir, str(ith_scan_to_transp) + ".pth")
                    ...
                    with torch.no_grad():
                        ...
                        list_metrics.append(dict_metrics)
    # save
    pd.DataFrame.from_records(list_metrics).to_csv(os.path.join(transport_general_covariate_meshes_dir, 'metrics.csv'))
